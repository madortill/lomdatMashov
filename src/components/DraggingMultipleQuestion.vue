<template>
    <div id="dragging-multiple-question">
        <div class="draggable-container">
            <h1 class="title-questionDragging">{{ this.questionInfo.title }}</h1>
            <p class="text-drag"> * גרור את התשובה המתאימה ביותר.</p>
            ...
            <ul v-show="showWordWarehouse" class="word-warehouse" @dragstart="onDragging" @dragover="allowDrop" @drop="(e) => {
                ...
                <li v-for="(item, index) in this.questionInfo.QArray" :key="index" class="list-item" draggable="true"
...
export default {
    name: 'dragging-multiple-question',
    props: ['questionInfo'],
    components: {
    ...
    methods: {
        ...
        checkDraggable(ev) {
            ...
            for (let i = 0; i < this.answerArray.length; i++) {
                let listItem = document.querySelector(`.draggable-area .list-item:nth-of-type(${i + 1})`);
                if (listItem) {
                    if (this.questionInfo.correctArray[i] === this.answerArray[i]) {
            ...
            if (indexCorrectAns === (this.questionInfo.correctArray.length)) {
